...
import com.devfood.devfoodapi.domain.service.CadastroRestauranteService;
...
public class RestauranteController {
	...
	public Restaurante buscar(@PathVariable("restauranteId") Long id) {
		return restauranteService.buscarOuFalharPorId(id);
	}
	...
	public Restaurante adicionar(@RequestBody @Valid Restaurante restaurante){
		try {
			return restaurante = restauranteService.adicionar(restaurante);
	...
	public Restaurante atualizar(@PathVariable("restauranteId") Long id, @RequestBody Restaurante restaurante){
		@Valid Restaurante restauranteAtual = restauranteService.buscarOuFalharPorId(id);
		
		BeanUtils.copyProperties(restaurante, restauranteAtual, "id",
				"formasPagamento", "endereco", "dataCadastro");
		
		try{
			return restauranteAtual = restauranteService.adicionar(restauranteAtual);
		}catch(RestauranteNaoEncontradoException e) {
			throw new NegocioException(e.getMessage());
		}
	}
	...
	public Restaurante atualizarParcial(@PathVariable("restauranteId") Long id,
		...
		Restaurante restauranteAtualizado = restauranteAtual.get();
