...
import com.devfood.devfoodapi.domain.service.CadastroEstadoService;
...
public class EstadoController {
	...
	public Estado buscar(@PathVariable("estadoId") Long id) {
		return estadoService.buscarOuFalharPorId(id);
	}
	
	@PostMapping
	@ResponseStatus(HttpStatus.CREATED)
	public Estado adicionar(@RequestBody Estado estado) {
		return estado = estadoService.adicionar(estado);
	}
	
	@PutMapping("/{estadoId}")
	public Estado atualizar(@PathVariable("estadoId") Long id, @RequestBody Estado estado){
		Estado estadoAtual = estadoService.buscarOuFalharPorId(id);
		
		BeanUtils.copyProperties(estado, estadoAtual, "id");
		
		return estadoService.adicionar(estadoAtual);
